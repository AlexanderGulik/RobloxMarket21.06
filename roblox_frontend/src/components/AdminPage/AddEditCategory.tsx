...
const AddEditCategory = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      handleSetMessage('Пожалуйста, введите название категории', 'error');
      return;
    }
    if (!image) {
    ...
    try {
      ...
      handleSetMessage('Категория успешно добавлена', 'success');
      setName('');
      resetImage();
    } catch (error: any) {
      handleSetMessage(error.response?.data?.message || 'Ошибка при добавлении категории', 'error');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <Container maxWidth="sm" sx={{ px: isMobile ? 2 : 3 }}>
        ...
        >
          ...
          >
            ...
            >
              {isLoading ? <CircularProgress size={isMobile ? 20 : 24} color="inherit" /> : 'Добавить категорию'}
            </Button>
          </Box>
        </Paper>
      </Container>
