...
const ArchiveOrdersList: React.FC = () => {
  ...
  const fetchIncomeOrders = async () => {
    try {
      setIsLoading(true);
      const response = await AdminService.getIncomeOrders();
      ...
      if (axios.isAxiosError(error)) {
        ...
        handleSetMessage(axiosError.response?.data.message || 'Ошибка при получении входящих заказов', 'error');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handlePageChange = async (event: React.ChangeEvent<unknown>, page: number) => {
    ...
    try {
      ...
      setCurrentPage(page);
      const response = await AdminService.getIncomeOrders(page);
      setOrders(response.data.orders);
      setCurrentPage(response.data.currentPage);
      setTotalPages(response.data.totalPages);
    } catch (error) {
  ...
  const handleChangeOrderType = async (orderId: string) => {
    try {
      ...
      await AdminService.changeOrderTypeById(orderId, 'Архивный');
